...
loop	LDR R2, R0, #0	; Carico A[i]
	BRZ exit	; Se il numero � zero esco
	BRN neg		; Il primo numero caricato � negativo
	BRP pos		; Il primo numero caricato � positivo

neg	AND R3, R3, R3	; Verifico il segno del secondo numero
	BRN under	; Il secondo numero � negativo, ci pu� essere underflow
	ADD R3, R2, R3	; Il secondo numero � positivo, sommo
	STR R3, R1, #0	; Salvo la somma in R[i]
	BRNZP next

pos 	AND R3, R3, R3	; Verifico il segno del secondo numero
	BRP over	; Il secondo numero � positivo, ci pu� essere overflow
	ADD R3, R2, R3	; Il secondo numero � negativo sommo
	STR R3, R1, #0	; Salvo la somma in R[i]
	BRNZP next

under	ADD R3, R2, R3	; Sommo i due numeri
	BRP zero	; Se c'� underflow sommo 0
	STR R3, R1, #0	; Salvo la somma in R[i]
	BRNZP next

over 	ADD R3, R2, R3	; Sommo i due numeri
	BRN zero	; Se c'� overflow salvo 0
	STR R3, R1, #0	; Salvo la somma in R[i]
	BRNZP next
